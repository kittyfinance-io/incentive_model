export class EnvInputs {
  ...
  constructor(
    ...
    // this.total_share_supply = total_share_supply;
...
// let lion_share_mcap_current = 110000;
// let pre_launch_avax_kitty = 20450;
// let lion_price = 30;
// const envInputs = new EnvInputs(
//   17.22, // avax price
//   lion_price, // lion avax price
//   110000 /
//     // EnvInputs.total_share_supply *
//     (100 * protocolParams.protocol_tax_public_vs_dao_ratio), // lion share price
//   500000, // buying vol
//   250000, // selling vol
...
export const get_roi = (
  ...
  if (
    ...
    (e.totalSupplyLion() - p1_pol_lion - p3_pol_lion) * e.lion_price_in_dollar
  ) {
    let cap =
      e.buying_volume +
      (e.totalSupplyLion() - p1_pol_lion - p3_pol_lion) *
        e.lion_price_in_dollar;
    e.selling_volume = cap;
    console.log(
      `\nWARNING Selling volume is capped at circulating supply - POL! Capped to: ${Number(
        cap
      )}$\n`
    );
  }
  ...
  // --- mode agnostic ---
  let pol_increase_in_dollar =
    p.getLiqContrib(peg)! *
    (Math.max(e.buying_volume, e.selling_volume) -
      Math.abs(e.buying_volume - e.selling_volume)) *
    2;
  ...
  let price_appreciation_of_lion_in_percent =
    get_price_delta_lion(lion_to_add, avax_to_add, pol_increase_in_dollar) - 1;
  console.log(
    `Price delta of Lion in percent from trading: ${
      price_appreciation_of_lion_in_percent * 100
    }%`
  );
  ...
  let supply_delta_from_burn_reemissions =
    (-supply_delta_from_peg_tax_burns * p.peg_tax_re_mint_rate * e.timeframe) /
    p.reemission_timeframe;
  console.log(
  ...
  if (use_virtual) {
    ...
    new_lion_price =
      (1 + price_appreciation_of_lion_in_percent) * e.lion_price_in_dollar;
    console.log(`New lion price: ${new_lion_price}$`);

    net_price_appreciation_of_lion =
      (1 + price_appreciation_of_lion_in_percent) *
        investment_sum *
        new_peg_fee_factor -
      investment_sum;
    console.log(
  ...
  if (mode === "stake lion") {
    ...
    let total_avax_rewards_in_dollar =
      ...
      (supply_delta_from_burn_reemissions +
        ...
        new_lion_price;
    console.log(
      `\nGlobal Lion staking rewards: ${Number(
    ...
    console.log(
      `Share of lion staking pool: ${share_of_lion_staking_pool * 100}%`
    );

    let user_staking_rewards_in_dollar =
      share_of_lion_staking_pool * total_avax_rewards_in_dollar;
    console.log(
      `Staking rewards gained: ${Number(
        user_staking_rewards_in_dollar
      )}$ in AVAX`
    );
    ...
    console.log(
      `===================================== ROI: ${
        (user_staking_rewards_in_dollar + net_price_appreciation_of_lion) /
        investment_sum
      } ====================================`
    );
    return {
  ...
  if (mode === "stake shares") {
    ...
    // buy shares
    let user_lion_shares_amount =
      (user_lion_amount * e.lion_price_in_dollar) / e.share_price_in_dollar;
    ...
    let global_share_staking_rewards =
      (Math.max(e.buying_volume, e.selling_volume) -
        Math.abs(e.buying_volume - e.selling_volume)) *
        2 *
        p.getProtocolContribution(peg) +
      Math.abs(e.buying_volume - e.selling_volume) *
        p.getProtocolContribution((peg + new_peg) / 2);
    console.log(
      `\nGloabl share staking rewards: ${Number(global_share_staking_rewards)}$`
    );

    let dao_rewards =
      global_share_staking_rewards * p.protocol_tax_public_vs_dao_ratio;
    console.log(`DAO rewards received: ${Number(dao_rewards)}$`);
    console.log(
      `DAO marketing & running costs: ${Number(dao_rewards * p.marketing_cut)}$`
    );
    console.log(
      `DAO team: ${Number(dao_rewards * (1 - p.marketing_cut) * p.team_cut)}$`
    );
    console.log(
      `DAO creator: ${Number(
        dao_rewards * (1 - p.marketing_cut) * (1 - p.team_cut)
      )}$`
    );
    ...
    console.log(
      `===================================== ROI: ${
        share_staking_rewards / investment_sum
      } ====================================`
    );
    return {
      mode: mode,
  ...
  if (mode === "stake p1") {
    ...
    console.log(
      `Global unclaimed rewards in $ worth of Lion: ${
        unclaimed_rewards * new_lion_price
      }`
    );
    ...
    console.log(
      `Global claimed rewards in $ worth of Lion: ${
        claimed_rewards * new_lion_price
      }$`
    );

    let global_p1_staking_minting_rewards_in_lion =
      unclaimed_rewards + claimed_rewards;
    console.log(
      `Global P1 staking rewards in Lion (considering early withdrawals): ${global_p1_staking_minting_rewards_in_lion}`
    );

    let share_of_p1_staking_pool =
      investment_sum / (e.p1_staked_in_dollar + investment_sum);
    console.log(
    ...
    console.log(
      `Net price appreciation of Lion in LP: ${
        net_price_appreciation_of_lion / new_peg_fee_factor / 2
      }$`
    );
    // appreciated lion + stable avax part
    let virtual_new_lp_value =
      net_price_appreciation_of_lion / new_peg_fee_factor / 2 + investment_sum;
    ...
    let net_lp_price_appreciation_gains =
      ...
      investment_sum;
    console.log(
      `Net LP price appreciation gains: ${net_lp_price_appreciation_gains}$`
    );

    console.log(
      `===================================== ROI: ${
        (lp_staking_rewards + net_lp_price_appreciation_gains) / investment_sum
      } ====================================`
    );
    return {
      mode: mode,
  ...
  if (mode === "stake p3") {
    ...
    let global_p3_staking_minting_rewards_in_lion =
      ...
      vanilla_rewards * (1 - e.early_withdrawal_rate) * p.early_withdrawal_tax;
    let share_of_p3_staking_pool =
      investment_sum / (e.p3_staked_in_dollar + investment_sum);
  ...
  if (mode === "lock p1") {
    ...
    console.log(
      `Global L1 lock rewards in dollar worth of Lion: ${
        global_l1_staking_minting_rewards_in_lion * new_lion_price
      }`
    );
    ...
    console.log(
      `===================================== ROI: ${
        lp_staking_rewards / investment_sum - 1
      } ====================================`
    );
    return {
      mode: mode,
  ...
  if (mode === "lock p3") {
    ...
    let global_l3_staking_minting_rewards_in_lion =
      vanilla_rewards + early_withdrawal_fees;
    console.log(
      `Global L3 lock rewards: ${
        global_l3_staking_minting_rewards_in_lion * new_lion_price
      }`
    );
...
export const print_rois = (
  ...
  function print_roi(i: number, e: EnvInputs, p: ProtocolParams, mode: string) {
    ...
    let roi = getRoi!["roi"];
    console.log(
      `\n=====> ROI from investing ${i}$ into >${mode}<: ${roi * 100}% in ${
        e.timeframe
      } days ===== \n`
    );
  }
